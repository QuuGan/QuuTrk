...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        log_once("x {}".format(x.size()))
        x = self.conv1(x)
        log_once("x.conv1 {}".format(x.size()))
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        log_once("x.maxpool {}".format(x.size()))
        variational_features = []

        p1 = self.layer1(x)
        log_once("p1 {}".format(p1.size()))
        p2 = self.layer2(p1)
        log_once("p2 {}".format(p2.size()))
        p3 = self.layer3(p2)
        log_once("p3 {}".format(p3.size()))
        p4 = self.layer4(p3)
        log_once("p4 {}".format(p4.size()))
