...
from models.dcnv2 import DeformableConv2d as DCNv2
...
class ResDown(MultiStageFeature):
    def __init__(self, pretrain=False):
        ...
        self.unfix(0.0)
        self.nl = NONLocalBlock2D(256)
        self.conv = nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=0)
    ...
    def forward(self, x):
        ...
        e = self.relu(self.bn(self.conv(self.nl(output[1]))))
        e = self.relu(self.bn(self.conv(e)))
...
class Refine(nn.Module):
    def __init__(self):
        ...
        self.v0 = nn.Sequential(DCNv2(64, 16, 3, padding=1), nn.ReLU(),
                           DCNv2(16, 4, 3, padding=1),nn.ReLU())

        self.v1 = nn.Sequential(DCNv2(256, 64, 3, padding=1), nn.ReLU(),
                           DCNv2(64, 16, 3, padding=1), nn.ReLU())

        self.v2 = nn.Sequential(DCNv2(512, 128, 3, padding=1), nn.ReLU(),
                           DCNv2(128, 32, 3, padding=1), nn.ReLU())
        self.h2 = nn.Sequential(DCNv2(32, 32, 3, padding=1), nn.ReLU(),
                           nn.Conv2d(32, 32, 3, padding=1), nn.ReLU())

        self.h1 = nn.Sequential(DCNv2(16, 16, 3, padding=1), nn.ReLU(),
                           nn.Conv2d(16, 16, 3, padding=1), nn.ReLU())

        self.h0 = nn.Sequential(DCNv2(4, 4, 3, padding=1), nn.ReLU(),
